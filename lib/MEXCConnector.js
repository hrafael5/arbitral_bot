...
class MEXCConnector {
        ...
        this.futuresWs.on("message", (data) => {
            ...
            try {
                ...
                if (message.channel === "push.ticker") {
                    ...
                    this.logger.debug(`[MEXCConnector][Futuros WS] Confirmação de subscrição recebida para Futuros.`);
                } else {
    ...
    subscribeToSpotTickers(symbols = []) {
        ...
        const subscriptionMessage = {
            method: "sub.ticker",
            param: {
                symbol: symbols[0] // Assumindo que subscrevemos um símbolo por vez para este formato
            },
            req_id: `spot_sub_${Date.now()}`
        };
        this.spotWs.send(JSON.stringify(subscriptionMessage));
        this.logger.debug(`[MEXCConnector][Spot WS] Mensagem de subscrição enviada: ${JSON.stringify(subscriptionMessage)}`);
    ...
    async getFuturesContractDetail() {
        const url = `${this.futuresRestUrl}/detail`; // URL corrigida
