...
class ArbitrageEngine {
    ...
    processMarketUpdate(latestMarketStateFromMonitor) {
        ...
        const spotVsSpotEnabled = !!this.config.arbitrage?.enable_spot_vs_spot; // --- ADICIONADO ---
    ...
    signalOpportunity(details) {
        ...
        const isSpotToSpot = details.buyInstrument?.toUpperCase() === "SPOT" && details.sellInstrument?.toUpperCase() === "SPOT"; // --- ADICIONADO ---
        
        const futuresVsFuturesStrategyIsEnabled = !!this.config.arbitrage?.enable_futures_vs_futures;
        const spotVsSpotStrategyIsEnabled = !!this.config.arbitrage?.enable_spot_vs_spot; // --- ADICIONADO ---

        // Filtro para garantir que a estratégia está habilitada
        ...
        // --- INÍCIO DO NOVO BLOCO ---
            ...
            opportunity.fundingRate = null; // Não aplicável
            opportunity.futuresVolume24hUSD = null; // Não aplicável
        }
        // --- FIM DO NOVO BLOCO ---
