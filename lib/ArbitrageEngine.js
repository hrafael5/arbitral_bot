...
// para permitir a leitura de arquivos diretamente do disco.
const fs = require('fs');
...
class ArbitrageEngine {
    constructor(globalConfig, signaler, logger) {
        // A configuração inicial ainda é útil para a primeira execução
    ...
    processMarketUpdate(latestMarketStateFromMonitor) {
        ...
        // --- INÍCIO DA ALTERAÇÃO ---
        // Força a releitura do arquivo de configuração a cada ciclo.
        // Isso garante que o motor sempre tenha acesso às configurações mais recentes.
        try {
            // Define o caminho para o arquivo de configuração.
            // **IMPORTANTE**: Ajuste este caminho para corresponder à localização do seu arquivo!
            const configPath = path.join(__dirname, '../../config.json');
            const configFileContent = fs.readFileSync(configPath, 'utf8');
            // Atualiza a configuração do motor com o conteúdo do arquivo
            this.config = JSON.parse(configFileContent);
        } catch (error) {
            this.logger.error(`[ArbitrageEngine] Erro fatal ao ler ou analisar o arquivo de configuração: ${error.message}`);
            // Interrompe o ciclo atual para evitar operar com configurações antigas ou incorretas.
            return; 
        }

        // Agora, essas variáveis sempre terão os valores mais recentes do arquivo.
