...
class ArbitrageEngine {
    constructor(globalConfig, signaler, logger) {
        ...
        this.logger = logger;
        // Se 'min_profit_percentage' não estiver no conf.ini, o padrão agora é 0.0.
        ...
        if (this.config.arbitrage?.enable_futures_vs_futures) {
            this.logger.info(`[ArbitrageEngine] Futures vs Futures strategy is INITIALLY ENABLED.`);
        }
        if (this.config.arbitrage?.enable_spot_vs_spot) {
            this.logger.info(`[ArbitrageEngine] Spot vs Spot strategy is INITIALLY ENABLED.`);
        }
    }
    ...
    signalOpportunity(details) {
        // A verificação abaixo garante que apenas oportunidades com lucro líquido
        // maior ou igual ao mínimo configurado no conf.ini serão sinalizadas.
        ...
        const spotVsSpotStrategyIsEnabled = !!this.config.arbitrage?.enable_spot_vs_spot;

        // --- INÍCIO DA CORREÇÃO ---
        // Se a oportunidade for Spot-Futuros, nós a deixamos passar.
        // Para os outros tipos, verificamos se a estratégia correspondente está habilitada.
        if (isSpotToFutures) {
            // É uma oportunidade Spot vs Futuros (Lucro S), então prossiga.
        } else if (isFuturesToFutures && !futuresVsFuturesStrategyIsEnabled) {
            // É Futuros vs Futuros, mas a estratégia está desabilitada, então ignore.
            return;
        } else if (isSpotToSpot && !spotVsSpotStrategyIsEnabled) {
            // É Spot vs Spot, mas a estratégia está desabilitada, então ignore.
            return;
        } else if (!isSpotToFutures && !isFuturesToFutures && !isSpotToSpot) {
            // Não corresponde a nenhum tipo conhecido, ignore por segurança.
            return;
        }
        // --- FIM DA CORREÇÃO ---

        // Mantemos esta verificação como uma "sanidade" para evitar enviar dados claramente errados (ex: -90% de spread)
