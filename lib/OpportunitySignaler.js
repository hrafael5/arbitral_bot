...
class OpportunitySignaler {
    ...
    signal(opportunity) {
        this.logger.debug(`[OpportunitySignaler] Tentando sinalizar oportunidade para ${opportunity.pair} - ${opportunity.direction}. Net Spread: ${opportunity.netSpreadPercentage.toFixed(4)}%`);
        const now = Date.now();
        ...
        if (now - lastSignal < this.signalCooldown) {
            this.logger.debug(`[OpportunitySignaler] Oportunidade para ${opportunity.pair} - ${opportunity.direction} ignorada devido ao cooldown. Tempo restante: ${this.signalCooldown - (now - lastSignal)}ms`);
            return;
        }
