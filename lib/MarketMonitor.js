    ...
    async pollSpotData(exchangeName, connector) {
        try {
            ...
            if (updatedCount > 0) {
                ...
                this.engine.invalidateOldOpportunities(currentOpportunityKeys, exchangeName, 'SPOT');
    ...
    async pollFuturesData(exchangeName, connector) {
        try {
            ...
            if (updatedCount > 0) {
                ...
                this.engine.invalidateOldOpportunities(currentOpportunityKeys, exchangeName, 'FUTURES');
                if (this.broadcastMarketData) this.broadcastMarketData();
            }
        } catch (error) {
