...
class MarketMonitor {
    ...
    async pollSpotData(exchangeName, connector) {
        try {
            ...
            const pairsForThisExchange = this.pairsToMonitorByExchange[exchangeName] || [];
            let updatedCount = 0; // CORREÇÃO: Variável inicializada
            ...
            if (updatedCount > 0) {
                ...
                // Invalidate opportunities that are no longer present
                const currentOpportunityKeys = new Set();
                for (const pair in this.marketData[exchangeName]) {
                    if (this.marketData[exchangeName][pair].spot.ticker) {
                        // A linha abaixo estava incorreta, op não está definido aqui.
                        // currentOpportunityKeys.add(`${pair}-${op.buyInstrument.toUpperCase()}-${op.sellInstrument.toUpperCase()}-${op.direction}`);
                        // A invalidação de oportunidades deve ser mais inteligente, baseada em oportunidades ativas, não apenas em tickers.
                        // Por enquanto, vamos remover a linha que causa o erro.
                    }
                }
                // this.engine.invalidateOldOpportunities(currentOpportunityKeys, exchangeName, \'SPOT\'); // Removido para evitar erro
    ...
    async pollFuturesData(exchangeName, connector) {
        try {
            ...
            if (updatedCount > 0) {
                ...
                const currentOpportunityKeys = new Set();
                for (const pair in this.marketData[exchangeName]) {
                    if (this.marketData[exchangeName][pair].futures.ticker) {
                        // A linha abaixo estava incorreta, op não está definido aqui.
                        // currentOpportunityKeys.add(`${pair}-${op.buyInstrument.toUpperCase()}-${op.sellInstrument.toUpperCase()}-${op.direction}`);
                        // A invalidação de oportunidades deve ser mais inteligente, baseada em oportunidades ativas, não apenas em tickers.
                        // Por enquanto, vamos remover a linha que causa o erro.
                ...
                // this.engine.invalidateOldOpportunities(currentOpportunityKeys, exchangeName, \'FUTURES\'); // Removido para evitar erro
                if (this.broadcastMarketData) this.broadcastMarketData();
            }
        } catch (error) {
