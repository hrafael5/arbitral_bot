...
class MarketMonitor {
    ...
    _initializeMarketData(exchange, pair) {
        if (!this.marketData[exchange]) {
            ...
            this.logger.debug(`[MarketMonitor] Inicializando estrutura para exchange: ${exchange}`);
        }
        if (!this.marketData[exchange][pair]) {
            ...
            this.logger.debug(`[MarketMonitor] Inicializado dados para par: ${pair} na exchange ${exchange}. Spot API: ${spotSymbolApi}, Futures API: ${futuresSymbolApi}`);
        }
    }
    ...
    start() {
        ...
        Object.entries(this.pairsToMonitorByExchange).forEach(([exchange, pairs]) => {
            this.logger.debug(`[MarketMonitor] Processando ${pairs.length} pares para ${exchange}.`);
        ...
        if (this.connectors.mexc) {
            ...
            if (mexcPairs.length > 0) {
                ...
                this.logger.debug(`[MarketMonitor] MEXC Spot Symbols para subscrição: ${spotSymbols.join(', ')}`);
                this.logger.debug(`[MarketMonitor] MEXC Futures Symbols para subscrição: ${futuresSymbols.join(', ')}`);

                this.connectors.mexc.connectSpotWebSocket(spotSymbols);
                this.connectors.mexc.connectFuturesWebSocket(futuresSymbols);
            } else {
                this.logger.warn("[MarketMonitor] Nenhum par MEXC para monitorar via WebSocket.");
            }
        }
    ...
    handleRealTimeUpdate(exchangeName, instrumentType, data) {
        this.logger.debug(`[MarketMonitor] Recebido update: Exchange=${exchangeName}, Tipo=${instrumentType}, Data=${JSON.stringify(data)}`);
        ...
        if (!pair) {
            this.logger.warn(`[MarketMonitor] Ignorando atualização para símbolo não monitorado: ${symbolApi} em ${exchangeName}/${instrumentType}`);
            return; // Ignora atualização de um par que não monitoramos
        }
        ...
        if (instrumentType === 'spot') {
            ...
            this.logger.debug(`[MarketMonitor] Atualizado Spot ticker para ${exchangeName}/${pair}: ${JSON.stringify(pairData.spot.ticker)}`);
            ...
            this.logger.debug(`[MarketMonitor] Atualizado Futures ticker para ${exchangeName}/${pair}: ${JSON.stringify(pairData.futures.ticker)}`);
        }
        ...
        this.logger.debug(`[MarketMonitor] Disparado processMarketUpdate e broadcast para ${exchangeName}/${pair}.`);
        if (this.broadcastMarketData) this.broadcastMarketData();
    }

    async pollSpotData(exchangeName, connector) {
        this.logger.debug(`[MarketMonitor] Polling Spot data para ${exchangeName}...`);
        try {
            ...
            if (!spotTickersMap || spotTickersMap.size === 0) {
                this.logger.warn(`[MarketMonitor] Nenhum ticker Spot recebido de ${exchangeName}.`);
                return;
            }
            ...
            pairsForThisExchange.forEach(pair => {
                ...
                if (tickerData) {
                    ...
                    this.logger.debug(`[MarketMonitor] Nenhum ticker Spot encontrado para ${exchangeName}/${pair} (símbolo: ${spotSymbol}).`);
                }
    ...
    async pollFuturesData(exchangeName, connector) {
        this.logger.debug(`[MarketMonitor] Polling Futures data para ${exchangeName}...`);
        try {
            const futuresDataMap = await connector.getAllFuturesBookTickers();
            if (!futuresDataMap || futuresDataMap.size === 0) {
                this.logger.warn(`[MarketMonitor] Nenhum ticker Futures recebido de ${exchangeName}.`);
                return;
            }
            ...
            pairsForThisExchange.forEach(pair => {
                ...
                if (tickerData) {
                    ...
                    this.logger.debug(`[MarketMonitor] Nenhum ticker Futures encontrado para ${exchangeName}/${pair} (símbolo: ${futuresSymbol}).`);
                }
            });
