...
function toggleBlock(opKey, opDataSnapshotString) {
  ...
  if (blockedItemIndex > -1) {
    ...
    try {
        ...
        console.error("Erro ao fazer parse do snapshot para bloquear:", e, opDataSnapshotString);
    }
  }
  saveBlockedOps();
  requestUiUpdate();
...
function renderOpportunitiesTable() {
    ...
    const favoritedOpWrappers = filteredOpWrappers.filter(opWrapper => state.favoritedOps.includes(`${opWrapper.data.pair}-${opWrapper.data.direction}`));
    const normalOpWrappers = filteredOpWrappers.filter(opWrapper => !state.favoritedOps.includes(`${opWrapper.data.pair}-${opWrapper.data.direction}`));
      ...
      try {
        ...
        if (currentDefaultCapital > 0 && op.buyPrice > 0) {
            ...
            if (numericQtd > 0) {
                ...
                qtyCellContent = `${displayQty} <button class="js-copy-qty" data-copy-value="${copyValue}">📋</button>`;
            } else {
                qtyCellContent = '0';
            }
        }
        ...
        const openAllIcon = `<svg class="open-exchange-icon js-open-all" ${dataAttributes} viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2.5" fill="none" stroke-linecap="round" stroke-linejoin="round" title="Abrir gráficos, calculadora E copiar qtd. sugerida"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg>`;
        const compraLink = `<a href="${getExchangeUrl(op.buyExchange, op.buyInstrument, op.pair)}" target="_blank" rel="noopener noreferrer" class="exchange-link">${getExchangeTag(op.buyExchange)} ${op.buyInstrument}<span>${formatPrice(op.buyPrice)}</span></a>`;
        const vendaLink = `<a href="${getExchangeUrl(op.sellExchange, op.sellInstrument, op.pair)}" target="_blank" rel="noopener noreferrer" class="exchange-link">${getExchangeTag(op.sellExchange)} ${op.sellInstrument}<span>${formatPrice(op.sellPrice)}</span></a>`;
        const calculatorIcon = `<svg class="calculator-icon js-open-calculator" ${dataAttributes} xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" title="Abrir Calculadora Detalhada em nova janela"><rect x="4" y="2" width="16" height="20" rx="2" ry="2"></rect><line x1="8" y1="6" x2="16" y2="6"></line><line x1="16" y1="14" x2="16" y2="18"></line><line x1="16" y1="10" x2="16" y2="10"></line><line x1="12" y1="10" x2="12" y2="10"></line><line x1="8" y1="10" x2="8" y2="10"></line><line x1="12" y1="14" x2="12" y2="18"></line><line x1="8" y1="14" x2="8" y2="18"></line></svg>`;

        tableHtml += `<tr>
          <td class="pair-cell">
            <span class="favorite-star js-toggle-favorite ${isFavorited ? 'favorited' : 'not-favorited'}" ${dataAttributes} title="${isFavorited ? 'Desfavoritar' : 'Favoritar'}">${isFavorited ? '★' : '☆'}</span>
            <span class="block-icon not-blocked js-toggle-block" ${dataAttributes} title="Bloquear">🚫</span>
            ${openAllIcon}
          ...
          console.error("Erro ao renderizar uma linha da tabela:", error, opWrapper?.data);
      }
    });
...
function setupEventListeners() {
  ...
  document.querySelectorAll('th.sortable').forEach(th => {
      th.addEventListener('click', (e) => {
          ...
          const columnKeyMatch = onclickAttr ? onclickAttr.match(/sortByColumn\('([^']+)'\)/) : null;
