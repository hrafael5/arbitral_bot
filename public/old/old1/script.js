...
function getFilteredOpportunities() {
    let filteredOpWrappers = state.arbitrageOpportunities.filter(opWrapper => {
      ...
      // Alteração: Não mostrar pares vigiados na tabela principal de oportunidades
...
function renderWatchedPairsTable() {
    ...
    state.watchedPairsList.forEach(pair => {
        const opportunitiesForPair = state.arbitrageOpportunities.filter(opWrapper => opWrapper.data.pair === pair);
        ...
        if (opportunitiesForPair.length > 0) {
            opportunitiesForPair.forEach(opWrapper => {
                ...
                tableHtml += `
                    <tr>
                        <td class="pair-cell">
                            <button class="remove-watched-pair" data-pair="${escapedPair}" title="Remover ${escapedPair} da vigilância">&times;</button>
                            ${getCurrencyIcon(pair)}
                            ${escapedPair}
                        </td>
                        <td>${getExchangeTag(op.buyExchange)} ${op.buyInstrument}<div>${formatPrice(op.buyPrice)}</div></td>
                        <td>${getExchangeTag(op.sellExchange)} ${op.sellInstrument}<div>${formatPrice(op.sellPrice)}</div></td>
             ...
             tableHtml += `<tr><td colspan="8" class="no-data">Aguardando dados de mercado para ${pair}...</td></tr>`;
        }
    });
