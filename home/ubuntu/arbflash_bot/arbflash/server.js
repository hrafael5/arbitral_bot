...
const MEXCConnectorWS = require('./lib/MEXCConnectorWS');
const GateConnectorWS = require('./lib/GateConnectorWS');
...
const connectors = { mexc: new MEXCConnectorWS(config.mexc, logger), gateio: new GateConnectorWS(config.gateio, logger) };
...
async function initializeAndStartBot() {
    ...
    try {
        ...
        await Promise.all([
            connectors.mexc.initialize(),
            connectors.gateio.initialize()
        ]);

        const [mexcPairs, gateioPairs] = await Promise.all([
            connectors.mexc.getSpotPairs().then(res => res.data.map(p => p.replace("USDT", "/USDT"))),
            connectors.gateio.getSpotPairs().then(res => res.data.map(p => p.replace("USDT", "/USDT")))
        ]);
