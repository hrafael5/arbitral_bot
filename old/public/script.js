...
const WATCHED_PAIRS_STORAGE_KEY = 'arbitrageDashboard_watchedPairs_v1';
...
const watchedPairsDisplayAreaEl = document.getElementById('watched-pairs-display-area');
...
function loadWatchedPairs() { 
  ...
  state.watchedPairsList = stored ? (JSON.parse(stored) || []) : []; 
  renderWatchedPairsDisplay(); 
}
...
function renderWatchedPairsDisplay() { 
  if (!watchedPairsDisplayAreaEl || !watchedPairsCountEl) return; 
  watchedPairsCountEl.textContent = state.watchedPairsList.length; 
  watchedPairsDisplayAreaEl.innerHTML = state.watchedPairsList.map(pair => 
    `<span class="watched-pair-tag">${escapeHTML(pair)} <button class="remove-watched-pair" data-pair="${escapeHTML(pair)}" title="Remover ${escapeHTML(pair)} da vigilância">&times;</button></span>`
  ).join(''); 
...
function formatTimeAgo(firstSeenTimestamp) { 
  if (!firstSeenTimestamp) return "N/A"; 
  const diff = Date.now() - firstSeenTimestamp; 
...
function renderOpportunitiesTable() {
    ...
    const watchedOpWrappers = filteredOpWrappers.filter(opWrapper =>
        state.watchedPairsList.includes(opWrapper.data.pair)
    );
    const favoritedOpWrappers = filteredOpWrappers.filter(opWrapper => {
        const opKey = `${opWrapper.data.pair}-${opWrapper.data.direction}`;
        return state.favoritedOps.includes(opKey) && !state.watchedPairsList.includes(opWrapper.data.pair);
    });
    ...
    [watchedOpWrappers, favoritedOpWrappers, normalOpWrappers].forEach(arr => arr.sort(sortFunction));

    const finalSortedOpportunities = [...watchedOpWrappers, ...favoritedOpWrappers, ...normalOpWrappers];
    let finalOpportunitiesToRender = finalSortedOpportunities.slice(0, state.maxOpportunitiesToShow);

    if (filteredOpWrappers.length === 0) {
    ...
    finalOpportunitiesToRender.forEach((opWrapper) => {
        ...
        const compraLink = `<a href="#" class="exchange-link" onclick="window.open(getExchangeUrl('${escapedBuyEx}', '${escapedBuyInst}', '${escapedPair}'), '_blank'); return false;">${getExchangeTag(op.buyExchange)} ${op.buyInstrument}<div>${formatPrice(op.buyPrice)}</div></a>`;
        const vendaLink = `<a href="#" class="exchange-link" onclick="window.open(getExchangeUrl('${escapedSellEx}', '${escapedSellInst}', '${escapedPair}'), '_blank'); return false;">${getExchangeTag(op.sellExchange)} ${op.sellInstrument}<div>${formatPrice(op.sellPrice)}</div></a>`;
    ...
    state.soundPlayedForVisibleOps.forEach(playedOpKey => {
        if (![...finalOpportunitiesToRender.keys()].some(opWrapper => `${opWrapper.data.pair}-${opWrapper.data.direction}` === playedOpKey)) {
  ...
  blockedOpsTableBodyEl.innerHTML = sortedBlockedOps.map(blockedOpItem => {
    ...
    return `
      <tr>
        ...
        <td>${getExchangeTag(snapshot.buyExchange)} ${snapshot.buyInstrument}<div>${formatPrice(snapshot.buyPrice)}</div></td>
        <td>${getExchangeTag(snapshot.sellExchange)} ${snapshot.sellInstrument}<div>${formatPrice(snapshot.sellPrice)}</div></td>
...
function toggleMonitorPares() {
  ...
  if (monitorParesTableContainerEl && monitorParesToggleIconEl) {
    ...
    monitorParesToggleIconEl.textContent = state.isMonitorParesExpanded ? '[-]' : '[+]';
...
function setupEventListeners() {
  ...
  if (filterEnableFutFutEl) {
      filterEnableFutFutEl.addEventListener('change', (e) => {
          ...
          .then(data => state.config.arbitrage.enableFuturesVsFutures = data.newConfig.enableFuturesVsFutures)
  ...
  if (filterEnableSpotSpotEl) {
      filterEnableSpotSpotEl.addEventListener('change', (e) => {
          ...
          .then(res => res.ok ? res.json() : Promise.reject('Update failed'))
          .then(data => state.config.arbitrage.enableSpotVsSpot = data.newConfig.enableSpotVsSpot)
          .catch(() => alert('Erro ao atualizar config no backend.'));
      });
  }
